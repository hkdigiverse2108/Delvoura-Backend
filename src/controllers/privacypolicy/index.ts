...
export const add_edit_privacy_policy = async (req, res) => {
  ...
  try {
    ...
    const response = await updateData(privacyPolicyModel, { isDeleted: false }, value, { upsert: true });
...
export const get_privacy_policy = async (req, res) => {
  reqInfo(req);
  try {
    const response = await privacyPolicyModel.findOne({ isDeleted: false }).lean();

    if (!response) return res.status(HTTP_STATUS.NOT_FOUND).json(new apiResponse(HTTP_STATUS.NOT_FOUND, responseMessage.getDataNotFound("privacy policy"), {}, {}));
